...
public abstract class FrameworkServlet extends HttpServletBean {
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		if (!this.refreshEventReceived) {
			...
			//调用DispatcherServlet的onRefresh方法，方法内调用initStrategies方法
			onRefresh(wac);
		}
		//把当前建立的上下文存到ServletContext中，注意使用的属性名是和当前Servlet名相关的
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		/**
		 * 
		 * FrameworkServlet&ContextRefreshListener类，实现了ApplicationListener接口。
		 * AbstractApplicationContext类的refresh方法内的finishRefresh()方法，会调用publishEvent方法，publishEvent会调用所有实现了ApplicationListener的方法。进而会调用
		 * FrameworkServlet&ContextRefreshListener类onApplicationEvent方法，这个方法会触发DispatcherServlet的onRefresh方法。
