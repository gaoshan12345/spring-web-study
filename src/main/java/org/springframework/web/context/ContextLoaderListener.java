...
public class ContextLoaderListener extends ContextLoader implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent event) {
		logger.info("ContextLoaderListener contextInitialized ********** getContextPath" + event.getServletContext().getContextPath());
		this.contextLoader = createContextLoader();
		if (this.contextLoader == null) {
			/**
			 * 因为createContextLoader返回null，所以会执行到这里。
			 * 同时，ContextLoaderListener extends ContextLoader，所以可以执行this.contextLoader = this，所以调用的是ContextLoaderListener.initWebApplicationContext方法。
			 * 因为ContextLoaderListener并没有覆写initWebApplicationContext方法，所以最终调用的是ContextLoader类的initWebApplicationContext方法。
			 */
