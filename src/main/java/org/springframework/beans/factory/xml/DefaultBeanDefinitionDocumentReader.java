...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		//解析前处理，留给子类实现。本类有实现该方法，但是方法体内是空实现。子类应该会覆写此方法
		preProcessXml(root);
		...
		try {
			...
			if(transformer != null) { 
				try { 
					...
					transformer.transform(new DOMSource(node), new StreamResult(sw)); return sw.toString(); 
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		if (absoluteLocation) {
			try {
				//getReaderContext().getReader()获取的是XmlReaderContext对象的XmlBeanDefinitionReader reader属性
...
//		ele.getNodeName();
		if (bdHolder != null) {
			...
			try {
				...
				/**
				 * 这段是用于验证生成对象是代理的问题，
				 * 真实情况直接调用BeanDefinitionReaderUtils.registerBeanDefinition(bdHolder, getReaderContext().getRegistry());就可以
				 */
				
				if(bdHolder.getBeanName().equals("userManager")){
	...
	protected void preProcessXml(Element root) {
		logger.info("preProcessXml do nothing - wangwenlong");
	}
	...
	protected void postProcessXml(Element root) {
		logger.info("postProcessXml do nothing - wangwenlong");
	}
