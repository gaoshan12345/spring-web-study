...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	//创建bean方法createBean中会用到beanPostProcessors
	...
	protected <T> T doGetBean(
		 ...
		 * Spring创建单例bean的原则是不等bean创建完成就会将创建bean的ObjectFacroty提早曝光
		 * 也就是将ObjectFacroty加入到缓存中，一旦下个bean创建时候需要依赖上个bean则直接使用ObjectFacroty
		 */
		// Eagerly check singleton cache for manually registered singletons.
		//直接尝试从缓存获取或者singletonFactories中的ObjectFacroty中获取
		...
		if (sharedInstance != null && args == null) {
			...
			//返回对应的实例，有时候存在诸如ObjectFacroty的情况并不是直接返回实例本身而是返回指定方法返回的实例
			bean = getObjectForBeanInstance(sharedInstance, name, beanName, null);
		}

		else {
			//只有在单例情况才会尝试解决循环依赖，原型模式情况下，如果存在A中有B的属性，B中有A的属性，那么当依赖注入的时候，就会产生当A还未创建完的时候因为
			...
			//如果不是仅仅做类型检查则是创建bean，这里是进行记录
			if (!typeCheckOnly) {
				markBeanAsCreated(beanName);
			}

			try {
				//将存在XML配置文件的GernericBeanDefinition转化为RootBeanDefinition，如果指定BeanName是子Bean的话同时会合并父类的相关属性
				final RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
				checkMergedBeanDefinition(mbd, beanName, args);

				/**
				 * 获取当前bean的所有依赖bean，这样会触发getBean的递归调用，直至渠道一个没有任何依赖的bean为止
				 */
				...
				/**
				 * 这里创建Singleton bean的实例，通过调用createBean方法，这里有一个
				 * 回调函数getObject，会在getSingleton中去调用ObjectFactory的createBean
				 */
					...
					sharedInstance = getSingleton(beanName, new ObjectFactory<Object>() {
						public Object getObject() throws BeansException {
							try {
								...
								logger.info("AbstractBeanFactory call AbstractAutowireCapableBeanFactory.createBean("+beanName+", mbd, args)");
								//调用AbstractAutowireCapableBeanFactory.createBean(final String beanName, final RootBeanDefinition mbd, final Object[] args)
	...
	public void addBeanPostProcessor(BeanPostProcessor beanPostProcessor) {
		...
		logger.info("AbstractBeanFactory add beanPostProcessor " + beanPostProcessor.getClass());
	...
	protected RootBeanDefinition getMergedLocalBeanDefinition(String beanName) throws BeansException {
		...
		//DefaultListableBeanFactory.getBeanDefinition(String beanName)方法
